...
public final class H2ConnectionUtils {
    ...
    static {
        try {
            ...
            if (resourceAsStream != null) {
                ...
                throw new MissingResourceException("configuration.properties not found", H2ConnectionUtils.class.getName(), "");
            }
        } catch (IOException e) {
    ...
    public static void initDatabase(final HikariDataSource dataSource) {
        try (Connection connection = dataSource.getConnection();) {
            connection.createStatement().execute(new Scanner(Thread.currentThread().getContextClassLoader().getResourceAsStream("Rooms.sql")).useDelimiter("\\Z").next());
